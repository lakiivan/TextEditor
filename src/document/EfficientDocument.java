...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(String word : tokens) {
			if(isWord(word)) {
				numWords++;
				numSyllables += Document.countSyllables(word);
			} else {
				numSentences++;
			}
		}
		
		//check if  the last word is not punctuation and add if it is not add +1 to numSentences
		if(tokens.size() > 0) {
			if(tokens.get(tokens.size()-1).indexOf('!') == -1 && tokens.get(tokens.size()-1).indexOf('?') == -1 && tokens.get(tokens.size()-1).indexOf('.') == -1) {
				numSentences++;
			}
		}
	}
