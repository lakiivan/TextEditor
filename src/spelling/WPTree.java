...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		String curr_word = word1;
		List<WPTreeNode> queue = new LinkedList<WPTreeNode>();
		List<String> visited = new LinkedList<String>();
		WPTreeNode wps = new WPTreeNode(word1, null);
		queue.add(wps);
		visited.add(curr_word);
		while(!queue.isEmpty() && !curr_word.equals(word2) && queue.size() < 1000) {
			System.out.println("queue size: " + queue.size());
			WPTreeNode curr_tn = queue.remove(0);
			curr_word = curr_tn.getWord();
			System.out.println("Current word: " + curr_word + "/" + word2);
			if(curr_word.equals(word2)) {
				List<String> path = curr_tn.buildPathToRoot();
				System.out.println("BINGO!!!!!!");
				return path;
			}	
			List<String> dist_one = nw.distanceOne(curr_word, true);
			for(String s : dist_one) {
				WPTreeNode temp = curr_tn.addChild(s);
				String temp_word = temp.getWord(); 
